...
!Definitions
  ...
  NGAUSS = CEILING((dble(NORDER) + 1)/2) + 25	!for p-FEM (the +1 is not 
  ...
  !Initialize 
    !for Explicit RKM-4
    ! a_RKM(2,1) = 1.0D0/2.0D0
    ! a_RKM(3,1) = 0.0D0
    ! a_RKM(3,2) = 1.0D0/2.0D0
    ! a_RKM(4,1) = 0.0D0
    ! a_RKM(4,2) = 0.0D0
    ! a_RKM(4,3) = 1.0D0
    ! c_RKM(1) = 0.0D0
    ...
    c_RKM(1) = 0.0D0
    c_RKM(2) = alpha
    b_RKM(1) = 1.0D0-(1.0D0/(2.0D0*alpha))
    b_RKM(2) = (1.0D0/(2.0D0*alpha))
  !~~~~~~~~~~~~~~~~~~~~TimeLoop~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~!Starts
  Do Istep = 1,n_istep
    write(*,*)'~~~~~~~~~~~~~~~~~~~'
    ...
    !Compute k vectors for RKM
      Do I = 1,RKM_Order
        ...
        !get k_v
          ...
          !calculate (c*c*<BC,w> + <f,w>)  at t = tn + c(i)*dt  
          call getRHS(TOTELS,NODEL,NORDER,NGAUSS,ofst_ELDAT,ELDAT,&
                   ...
                   NODEF,BETA,ENR_Type,&
                   c_RKM(I))
          !calculate [M]^(-1)*(c*c*<BC,w> + <f,w>) == R_tilde
          call MATMULCPLX(invGM,TOTDOF,TOTDOF,&
                      RHS,TOTDOF,1,&
